  ...
  try {
    ...
    if (errorMessage.includes('timeout')) {
      ...
      res.status(401).json({
        ...
        reason: "Invalid or missing API key. Please check your OpenRouter API key configuration.",
        details: errorMessage
      });
    } else {
      res.status(500).json({
...
app.post("/token", async (req, res) => {
  try {
    ...
    // Subtract 60 seconds from current time to account for potential clock drift
    const now = Math.floor(Date.now() / 1000);
    const issuedAt = now - 60; // 60 seconds ago to avoid timing issues
    const expiration = now + 60 * 60; // 1 hour from now

    console.log(`[TOKEN] Generating token for ${userId} - issued: ${issuedAt}, expires: ${expiration}, now: ${now}`);
